...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/koopichi/x-ui/refs/heads/master/install_en.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/koopichi/x-ui/blob/master/x-ui_en.sh
    if [[ $? != 0 ]]; then
        echo ""
